...
cmd = torch.CmdLine()
cmd:option('-data', '../cnn/', [[path to the data folder containing train, val. & test records]])
...
cmd:option('-question_pad', 'left', [[which side to pad the question to make sequences in a batch 
                                                            to be of same size? left or right? ]])
cmd:option('-passage_pad', 'right', [[which side to pad the passage to make sequences in a batch 
                                                            to be of same size? left or right? ]])
...
function gen_tensors(folder, label)
  ...
  for i = 1, #d2plen, params.batch_size do
    ...
    if cur_batch_size ~= params.batch_size then break end   -- (to-do)
    local max_passage, max_ans_cand, max_question = get_cur_batch_stat(i, i + cur_batch_size - 1)
    local passage_batch_tensor, passage_rev_batch_tensor, question_batch_tensor, question_rev_batch_tensor, answer_cand_batch_tensor = 
              torch.CudaTensor(cur_batch_size, max_passage):fill(0), torch.CudaTensor(cur_batch_size, max_passage):fill(0),
              torch.CudaTensor(cur_batch_size, max_question):fill(0), torch.CudaTensor(cur_batch_size, max_question):fill(0),
    ...
    for j = 1, cur_batch_size do
      ...
      for k = 1, (#record[1])[1] do
        if params.passage_pad == 'left' then
          passage_batch_tensor[j][k] = record[1][k]
          passage_rev_batch_tensor[j][(#record[1])[1] - k + 1] = record[1][k]
        else
          passage_batch_tensor[j][max_passage - k + 1] = record[1][k]
          passage_rev_batch_tensor[j][max_passage - (#record[1])[1] + k] = record[1][k]
        end
      end
      for k = 1, (#record[2])[1] do
        if params.question_pad == 'left' then
          question_batch_tensor[j][k] = record[2][k]
          question_rev_batch_tensor[j][(#record[2])[1] - k + 1] = record[2][k]
        else
          question_batch_tensor[j][max_passage - k + 1] = record[2][k]
          question_rev_batch_tensor[j][max_passage - (#record[2])[1] + k] = record[2][k]
        end
      end
    ...
    table.insert(batches, {{passage_batch_tensor, passage_rev_batch_tensor}, {question_batch_tensor, question_rev_batch_tensor},
                                                                                 answer_cand_batch_tensor, answer_batch_tensor})
  end
...
params.train_batches = gen_tensors(params.train_folder, 'train')
params.valid_batches = gen_tensors(params.val_folder, 'valid')
params.test_batches = gen_tensors(params.test_folder, 'test')
