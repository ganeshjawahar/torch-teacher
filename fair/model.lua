...
function get_model(params)
  ...
  local support_mem_model = nn.Sequential():add(nn.Identity()):add(nn.SplitTable(1, 1))
                                               :add(support_dict_master)
  if params.window_compo == 'sum' then 
    support_mem_model:add(nn.CAddTable())
  else
    support_mem_model:add(nn.JoinTable(1, 1))
  end
  ...
  local query_mem_model = nn.Sequential():add(nn.Identity()):add(nn.SplitTable(1, 1))
                                        :add(query_dict_master)
  if params.window_compo == 'sum' then 
    query_mem_model:add(nn.CAddTable())
  else
    query_mem_model:add(nn.JoinTable(1, 1))
  end
